...
func FetchKeyAndEndpoint() (string, string, error) {
    if err := requireAz(); err != nil { return "", "", err }
    pm, err := profiles.NewManager()
    if err != nil { return "", "", err }
    cfg, err := pm.GetCurrentConfig("dev")
    if err != nil { return "", "", err }
    if err := pm.Validate(cfg); err != nil { return "", "", err }
    if err := runCmd("az", "account", "set", "--subscription", cfg.Subscription); err != nil { return "", "", err }
...
func GetEndpoint(subscription, resource, group string) (string, error) {
    if err := requireAz(); err != nil { return "", err }
    if err := runCmd("az", "account", "set", "--subscription", subscription); err != nil { return "", err }
