...
func InteractiveSelect(title, placeholder string, options []SelectOption, currentValue string) (string, error) {
    ...
    for i, opt := range options {
        if opt.ID == currentValue { cursor = i; break }
    }
    ...
    if res.cancelled { return "", fmt.Errorf("selection cancelled") }
...
func (m selectorModel) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
    ...
    case tea.KeyMsg:
        ...
        case tea.KeyEsc, tea.KeyCtrlC:
            m.quitting = true; m.cancelled = true; return m, tea.Quit
        case tea.KeyEnter:
            if len(m.filtered) > 0 { m.selected = m.filtered[m.cursor].ID; m.quitting = true; return m, tea.Quit }
        case tea.KeyUp:
...
func (m selectorModel) View() string {
    ...
    b.WriteString(titleStyle.Render(m.title)); b.WriteString("\n")
    ...
    max := 10
    start := m.cursor - max/2; if start < 0 { start = 0 }
    end := start + max; if end > len(m.filtered) { end = len(m.filtered); start = end - max; if start < 0 { start = 0 } }
    for i := start; i < end; i++ {
        opt := m.filtered[i]
        if i == m.cursor { b.WriteString(sel.Render("> " + opt.Display)) } else { b.WriteString(norm.Render("  " + opt.Display)) }
        b.WriteString("\n")
    }
    b.WriteString("\n")
...
func filterOptions(options []SelectOption, search string) []SelectOption {
    ...
    for _, o := range options {
        if strings.Contains(strings.ToLower(o.ID), s) || strings.Contains(strings.ToLower(o.Display), s) { out = append(out, o) }
    }
    return out
}
