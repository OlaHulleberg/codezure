...
var profilesCmd = &cobra.Command{
    ...
    RunE: func(cmd *cobra.Command, args []string) error {
        ...
        entries, err := os.ReadDir(pmProfilesDir(pm)); if err != nil { return err }
        current, _ := pm.GetCurrent()
        for _, e := range entries {
            if filepath.Ext(e.Name()) == ".json" {
                name := e.Name()[:len(e.Name())-5]
                mark := ""
                if name == current { mark = " *" }
                fmt.Printf("%s%s\n", name, mark)
            }
        }
        return nil
...
func pmDir(pm *profiles.Manager) string { home, _ := os.UserHomeDir(); return filepath.Join(home, ".codzure") }
...
var configSaveCmd = &cobra.Command{
    ...
    RunE: func(cmd *cobra.Command, args []string) error {
        pm, err := profiles.NewManager(); if err != nil { return err }
        cfg, err := pm.GetCurrentConfig(Version); if err != nil { return err }
        name := args[0]
...
var configSwitchCmd = &cobra.Command{
    ...
    RunE: func(cmd *cobra.Command, args []string) error {
        pm, err := profiles.NewManager(); if err != nil { return err }
        name := args[0]
        if _, err := os.Stat(filepath.Join(pmProfilesDir(pm), name+".json")); err != nil { return err }
...
var configDeleteCmd = &cobra.Command{
    ...
    RunE: func(cmd *cobra.Command, args []string) error {
        pm, err := profiles.NewManager(); if err != nil { return err }
        current, _ := pm.GetCurrent(); if current == args[0] { return fmt.Errorf("cannot delete current profile") }
...
var configCopyCmd = &cobra.Command{
    ...
    RunE: func(cmd *cobra.Command, args []string) error {
        ...
        b, err := os.ReadFile(filepath.Join(pmProfilesDir(pm), src+".json")); if err != nil { return err }
